...
import { sum } from "./sum";
import { RollIndexOutput } from "./types";
...
/** @deprecated use DiceDropKeepType */
export const DropKeepType = DiceDropKeepType;
...
export class DiceDropKeep {
	public constructor(protected core?: DiceDropKeepData) { }

	public get alias(): string { return this.core?.alias ?? ""; }
	public get isEmpty(): boolean { return !this.type || !this.value; }
	public get type(): DiceDropKeepType { return this.core?.type ?? DiceDropKeepType.None; }
	...
	public static parse(token: TokenData): DiceDropKeepData | undefined {
		if (token.key === "dropKeep") {
			...
			const type = [, "dl", "dh", "kl", "kh"].indexOf(alias);
