...
async function macroList(sageMessage: SageMessage): Promise<void> {
	...
	if (filtered.length) {
		...
		renderableContent.appendTitledSection(`<b>To view a macro, use:</b>`,
			`${sageMessage.prefix ?? ""}!!macro details name="${macros.first()?.name ?? "sword_attack"}"`,
			`${sageMessage.prefix ?? ""}!!macro list cat="${macros.find(macro => macro.category)?.category ?? "swords"}"`
			);
		return <any>sageMessage.send(renderableContent);
