...
function createSkillSelectRow(character: TPlayerCharacter): MessageActionRow {
	...
	selectMenu.addOptions({
		label: `Skill Roll: Untrained`,
		value: `Untrained`,
		default: activeSkill === `Untrained`
	});
...
function createRollButtonRow(character: TPlayerCharacter): MessageActionRow {
	...
	const edgeButtonColor = testEdgeSnag<MessageButtonStyleResolvable>(activeEdgeSnag, { edge:"SUCCESS", snag:"DANGER", none:"SECONDARY" });
	const edgeButton = createButton(`${character.id}|EdgeSnagBtn`, activeEdgeSnag ?? "Edge / Snag", edgeButtonColor);
	...
	return new MessageActionRow().addComponents(edgeButton, rollButton, rollSecretButton, rollInitButton);
}
...
const uuidActionRegex = /^(?:[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\|(?:View|Skill|EdgeSnag|EdgeSnagBtn|Roll|Secret|Init)$/i;
...
async function edgeSnagHandler(sageInteraction: SageInteraction, character: TPlayerCharacter, selectEdgeSnag?: Optional<TEdgeSnag>, buttonEdgeSnag?: Optional<TEdgeSnag>): Promise<void> {
	const activeEdgeSnag = testEdgeSnag(selectEdgeSnag, { edge:"Edge", snag:"Snag", none:undefined })
		?? testEdgeSnag(buttonEdgeSnag, { edge:"Snag", snag:undefined, none:"Edge" });
...
async function rollHandler(sageInteraction: SageInteraction<ButtonInteraction>, character: TPlayerCharacter, secret = false, init = false): Promise<void> {
...
async function sheetHandler(sageInteraction: SageInteraction): Promise<void> {
	...
	if (character) {
		const command = actionParts[1] as "View" | "Skill" | "EdgeSnag" | "EdgeSnagBtn" | "Roll" | "Secret" | "Init";
		switch(command) {
			case "View": return viewHandler(sageInteraction, character);
			case "Skill": return skillHandler(sageInteraction, character);
			case "EdgeSnag": return edgeSnagHandler(sageInteraction, character, sageInteraction.interaction.values[0]);
			case "EdgeSnagBtn": return edgeSnagHandler(sageInteraction, character, undefined, character.getSheetValue("activeEdgeSnag"));
