...
import { hpToGauge } from "../model/utils/hpToGauge.js";
...
function renderState(label: string, deck: Deck, opts?: { showHand?:boolean; showSpread?:boolean; auditMode?:boolean; }): string {
	const visualize = (count: number) => hpToGauge(Math.max(count, Math.ceil(0.125 * deck.cardCount)), deck.cardCount).slice(1);
