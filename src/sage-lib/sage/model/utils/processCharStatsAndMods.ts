...
export async function processCharStatsAndMods(char: GameCharacter, stats?: KeyValuePair<string, undefined>[], mods?:IncrementArg[]): Promise<StringSet> {
	const keysModdedAndUpdated = new StringSet();

	const updateStats = async (pairs: KeyValuePair<string, undefined>[]) => {
