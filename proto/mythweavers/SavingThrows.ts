...
export default abstract class SavingThrows {
	...
	public static for(pc: PlayerCharacter | PathbuilderCharacter | MythWeaversCharacter): SavingThrows {
		if (pc instanceof PathbuilderCharacter) {
			return new PbcSavingThrows(pc);
		}
		if (pc instanceof MythWeaversCharacter) {
			return new MwcSavingThrows(pc);
		}
...
class MwcSavingThrows extends SavingThrows {

	public constructor(private mwc: MythWeaversCharacter) { super(); }

	public getSavingThrow(savingThrow: TSavingThrow, ability: TAbility): Check {
		const check = new Check(this.mwc, savingThrow);
		check.level = this.mwc.getLevelMod(true);
		check.setAbility(ability);
		check.addProficiency(savingThrow);
		if (this.mwc.resilientBonus) {
			const desc = ["","Resilient", "Greater Resilient", "Major Resilient"][this.mwc.resilientBonus];
			check.addItemModifier(desc, this.mwc.resilientBonus);
		}
		return check;
	}
