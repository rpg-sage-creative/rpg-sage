...
function isTextChannel(channel: Optional<Channel>): boolean {
	if (channel) {
		if ("isText" in channel && typeof(channel.isText) === "function" && channel.isText()) return true;
		if ("isTextBased" in channel && typeof(channel.isTextBased) === "function" && channel.isTextBased()) return true;
	}
	return false;
}
...
export function toInviteUrl(guild: Optional<Guild>): OrNull<string> {
	...
	try {
		const bestInvite = guild.invites.cache.find(invite => {
			if ("stageInstance" in invite && invite.stageInstance) return false;
			if (invite.targetUser || invite.temporary) return false;
