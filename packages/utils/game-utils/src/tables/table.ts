...
import { rollDie } from "../dice/roll/rollDie.js";
import { DiceGroup } from "../dice/dice/DiceGroup.js";
import type { TDice } from "../dice/dice/Dice.js";
...
export class NextTableInfo {
	...
	public constructor(targetString: string, diceString?: Optional<string>, timesString?: Optional<string>) {
		const targetsArray = targetString.split("|") as [string],
			diceArray = (diceString ?? "").split("|") as [string],
			timesArray = (timesString ?? "").split("|");

		let neg = false;
		let target: string, dice: string, times: string;
		let parts: [string, string | undefined], desc: Optional<string>;
		...
		while (count--) {
			...
			parts = target.split("(") as [string, string | undefined];
			target = parts[0].trim();
			desc = parts.length > 1 ? parts[1]!.split(")")[0]!.trim() : null;

			const parsedDice = DiceGroup.parse(dice)?.children[0]!;
			this.items[count] = {
				tableName: target,
				dice: parsedDice,
				times: parsedDice.roll().total,
				descriptor: desc
			};
		}
