...
export abstract class SageCommandArgs<T extends SageCommand> {
	 ...
	 */
	public abstract getChannel(name: string): Optional<SupportedMessagesChannel>;
	...
	public getUrl(name: string): VALID_URL | null | undefined {
		...
		if (isDefined(url)) {
			return isUrl(url, { wrapChars:"<>", wrapOptional:true }) ? unwrap(url, "<>") as VALID_URL : null;
		}
		return url;
	}
